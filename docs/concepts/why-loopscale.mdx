...
# Why Loopscale
The advantages of the Loopscale protocol are best understood by explaining how the Limit Creditbook design differs from other lending protocols.
...
Despite these disadvantages, the pool-based model was designed to address a limitation of peer-to-peer models, where borrowers and lenders must wait for matching counterparties to enter the market when liquidity is insufficient. Loopscale introduces the Limit Creditbook (LCB) and virtualized Limit Creditbooks (vLCB) to overcome this limitation without compromising capital efficiency. The LCB allows borrowers and lenders to create orders (requests and offers) with customizable rates, collateral types, and durations. On Loopscale's interface, Loopscale, the LCB is abstracted to a series of vLCBs, cataloging orders with standardized terms to concentrate liquidity across predefined markets. As more vLCBs are introduced with finer parameterization, short-term liquidity growth is achieved while maintaining long-term flexibility and efficiency. For more information on the Limit Creditbook and other protocol features, refer to the [Protocol Concepts](./protocol-concepts/limit-creditbook.md) page.

## Reduced Rate Risk
On Loopscale, loans are direct matches between lender and borrower for a fixed length. Once a borrower accepts an offer, they’re locking in a rate for the length of the loan. This gives borrowers the opportunity to run more complex or longer term strategies that are otherwise difficult to run with float rate.
...
5. **Risk management**: In pool-based models, risk is pooled across assets and participants are subject to algorithmic and universal risk management. The Loopscale protocol offers unconstrained flexibility in loan structuring, pricing options, and collateral structuring, giving lenders full control over their risk profile.